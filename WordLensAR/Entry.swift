...
class Entry{
    //saving the URL as where to get the usdz model from Echo3D backend
    var downloadURL:String="";
    //entry ID of the model
    var entryID:String="";
    //server_url passed from Echo3D
    //typical value is: "https://api.echo3D.co/query?key=<API_Key>"
    var server_url:String="";
    //name of the uploaded file to echo3D
    var filename:String="";
    ...
    func loadScene(storage_loc:URL) -> SCNScene{
        ...
        print("load loc: \(self.storage_loc)")
    ...
    func downloadFile(completion: (URL) -> ()){
        ...
        if let pathComponent = url.appendingPathComponent(self.usdz_id) {
            ...
            if fileManager.fileExists(atPath: filePath) {
                ...
                print("download loc: \(self.storage_loc)")
                completion(savedURL);
                return;
            }else{
